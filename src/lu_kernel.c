...
int main(int argc, char * argv[])
{
    ...
    /* AMD to reduce fill-ins. It is noted that sometimes COLAMD may      */
    /* increase fill-ins and LU factorization time.                       */
    int amd = 1;
    ...
    /* better to run MC64, because the AMD pertumation vector is obtained  */
    /* without considering partial pivoting.                               */
    int rmvzero = 1;
    /* Desired running threads.                                            */
    int num_threads = atoi(argv[2]);
    /* Pivoting tolerance. It is recommended to set it to 1.0E-08.         */
    ...
    if (lu->_mode == REAL) glu_refact(lu, lu->_ax0, lu->_rhs0, 128, 128);
...
int glu_refact(LU * lu, double * nax, double * nrhs, int__t num_blocks, int__t num_threads_per_block)
{
    ...
    if (lu->_factflag != 1) {
        printf("[ ERROR ] Please do factorization before the GPU-based refactorization");
        return 0;
    }
    if (lu->_et == NULL) { lu->_et = coletree(lu); }
    glu_init(lu, nax, nrhs, num_blocks, num_threads_per_block);
    ...
    /*int__t j;
    for (j = 0; j < size; j++) {
        for (i = 0; i < lu->_L->_nz_count[j]; i++) {
            lu->_L->_values[j][i] = 0.;
        }
    }
    for (j = 0; j < size; j++) {
        for (i = 0; i < lu->_U->_nz_count[j]; i++) {
            lu->_U->_values[j][i] = 0.;
        }
    }*/
    ...
    refact_flag = glu_refact_kernel(lu, num_blocks, num_threads_per_block);
